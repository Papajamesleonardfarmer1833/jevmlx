...
POLICY_A = """PAYMENT RISK POLICY - apply the first matching rule
A1. 2FA bypassed, or the session was hijacked: fraud=true, risk=CRITICAL, action=BLOCK.
A2. Connection via anonymizing proxy or Tor exit node AND unrecognized device: fraud=true; risk=CRITICAL if amount >= 2000 USD, otherwise HIGH; action=BLOCK.
A3. Unrecognized device AND amount >= 2000 USD: fraud=true, risk=HIGH, action=BLOCK.
A4. No fraud under A1-A3, but any of: connection via anonymizing proxy or Tor exit node; unrecognized device; several payments in a short window (velocity burst); foreign merchant AND amount >= 2000 USD: fraud=false, risk=ELEVATED, action=REVIEW.
A5. Otherwise: fraud=false, risk=LOW, action=APPROVE."""

POLICY_B = """SUPPORT TRIAGE POLICY - apply the first matching rule
B1. Category: BILLING for charges, invoices, refunds and pricing; TECHNICAL for errors, crashes, outages, performance and integrations; ACCOUNT for sign-in, passwords, 2FA, permissions and profile; SHIPPING for delivery, tracking, parcels and addresses.
B2. Priority: P3 for questions, routine updates, cosmetic issues and feature requests; P1 if the issue blocks the customer's work and either affects more than one user or has no workaround; otherwise P2.
B3. needs_human=true if any of: priority is P1; a refund, credit or invoice correction above 100 USD is requested or warranted; the account requires unlocking or a security check; a parcel is lost, damaged or in dispute. Otherwise needs_human=false (an automated or self-service flow resolves it)."""
...
A_CASES = [
    dict(id="A01", amount=42.50, merchant="Harbor Market", country="US", country_name="United States",
         device="known", channel="residential", auth="ok", velocity="normal",
         note="Clear low: small domestic purchase on a known device."),
    dict(id="A02", amount=2400.00, merchant="Aurora Electronics", country="US", country_name="United States",
         device="new", channel="residential", auth="ok", velocity="normal",
         note="Rule A3: unrecognized device plus amount >= 2000."),
    dict(id="A03", amount=2600.00, merchant="Alpen Motoren", country="DE", country_name="Germany",
         device="known", channel="residential", auth="ok", velocity="normal",
         note="Rule A4: foreign merchant and high amount, but no device or transport signal."),
    dict(id="A04", amount=89.00, merchant="Streamly", country="US", country_name="United States",
         device="known", channel="proxy", auth="ok", velocity="normal",
         note="Rule A4: Tor alone, small amount and known device, so not fraud."),
    dict(id="A05", amount=3100.00, merchant="CipherTrade Exchange", country="CY", country_name="Cyprus",
         device="new", channel="proxy", auth="ok", velocity="burst",
         note="Rule A2: Tor plus unrecognized device plus high amount, the CRITICAL branch."),
    dict(id="A06", amount=19.99, merchant="PixelPlay Games", country="US", country_name="United States",
         device="known", channel="residential", auth="bypassed", velocity="normal",
         note="Rule A1: tiny amount but the session was hijacked."),
    dict(id="A07", amount=640.00, merchant="Nimbus Hosting", country="US", country_name="United States",
         device="known", channel="residential", auth="ok", velocity="burst",
         note="Rule A4: velocity burst without any takeover signal."),
    dict(id="A08", amount=120.00, merchant="Metro Pharmacy", country="US", country_name="United States",
         device="known", channel="residential", auth="ok", velocity="normal",
         note="Clear low: routine repeat merchant."),
    dict(id="A09", amount=1950.00, merchant="Britannia Jewels", country="GB", country_name="United Kingdom",
         device="new", channel="residential", auth="ok", velocity="normal",
         note="Rule A4: unrecognized device but amount is just under the 2000 threshold, so not fraud."),
    dict(id="A10", amount=300.00, merchant="QuickShip Courier", country="US", country_name="United States",
         device="known", channel="residential", auth="ok", velocity="borderline",
         note="Ambiguous: 3 payments in 6 minutes is only arguably a velocity burst."),
    dict(id="A11", amount=2450.00, merchant="LuxTime Jewelers", country="US", country_name="United States",
         device="verified", channel="residential", auth="ok", velocity="normal",
         note="Ambiguous: device is new but enrolled and verified through the bank app."),
    dict(id="A12", amount=890.00, merchant="Northline Travel", country="US", country_name="United States",
         device="known", channel="residential", auth="ok_new_phone", velocity="normal",
...
A_NAMES = ["Sarah Jenkins", "Daniel Okafor", "Mei Lin", "Tomas Novak", "Priya Raman",
           "Elena Petrova", "Marcus Webb", "Aisha Karim", "Jonas Berg", "Clara Moreau",
           "Victor Salazar", "Hannah Cole"]
A_CITIES = ["Seattle, WA", "Austin, TX", "Chicago, IL", "Denver, CO", "Portland, OR", "Boston, MA"]
A_IPS = ["73.162.44.18", "24.19.201.77", "98.114.6.203", "67.183.90.12", "47.210.55.31", "71.9.128.240"]
...
def render_a(f: dict) -> str:
    ...
    if f["device"] == "known":
        months = 8 + (i * 5) % 40
        prior = 20 + (i * 13) % 180
        device_text = (f"recognized cardholder device (enrolled {months} months ago, {prior} prior purchases)"
                       if style != 1 else
                       f"device recognition: known device, active for {months} months, {prior} prior transactions")
    elif f["device"] == "new":
        device_text = (f"UNRECOGNIZED device, first seen {4 + i * 3} minutes before the alert, no prior purchases"
                       if style != 1 else
                       f"device fingerprint not recognized: new device, first seen {4 + i * 3} minutes ago")
    else:  # verified
        device_text = ("device enrolled through the bank app 2 days ago after a verified one-time code; "
                       "no prior purchase on this device")
    if f["channel"] == "proxy":
        connection_text = "anonymizing proxy / Tor exit node (185.220.101.5, Frankfurt, Germany)"
    else:
        connection_text = f"residential ISP ({A_IPS[i % len(A_IPS)]}, {A_CITIES[i % len(A_CITIES)]})"
    auth_text = {
        "ok": "2FA completed on the phone number on file",
        "bypassed": "2FA bypassed: the session was hijacked with a stolen cookie",
        "ok_new_phone": ("2FA code approved on a phone number the cardholder replaced yesterday; "
    ...
    lines = [
        ...
        f"Cardholder: {A_NAMES[i % len(A_NAMES)]} ({years}-year account, average monthly spend ${A_AVG[i % len(A_AVG)]:,})",
        f"Merchant: {f['merchant']} ({f['country_name']}, {'domestic' if f['country'] == 'US' else 'foreign'} merchant)",
...
B_CASES = [
    dict(id="B01", scenario="duplicate_charge", money=29.99, scope="single", workaround=False,
         plan="Pro", who="Dana Whitfield, operations lead",
         ...
         body="Invoice INV-8891 lists two charges of USD 29.99 for the same billing period. "
              "I authorised one payment only. Please refund the duplicate charge to the card ending 4412.",
         note="Clear: wrong bill, refund under 100 USD."),
    dict(id="B02", scenario="outage", money=0.0, scope="production", workaround=False,
         plan="Enterprise", who="Marcus Vance, CTO",
         subject="Production API down: every checkout returns 502",
         body="Our production checkout integration has returned 502 Bad Gateway on every request for "
              "40 minutes. All customers are affected and no orders are completing. This is our busiest "
              "trading window and we have no workaround.",
         note="Clear: production outage, everyone blocked."),
    dict(id="B03", scenario="reset_email", money=0.0, scope="single", workaround=True,
         plan="Free", who="Ravi Patel, sole trader",
         subject="Password reset email never arrives",
         body="I request a reset link from the sign-in page and nothing arrives, not even in spam. I have "
    ...
    dict(id="B04", scenario="lockout", money=0.0, scope="team", workaround=False,
         plan="Team", who="Ingrid Halvorsen, workspace owner",
         subject="Workspace owner locked out after failed sign-in attempts",
         body="My account was locked after five failed sign-in attempts because my password manager had a "
              "stale entry. I am the workspace owner and my team of 12 cannot reach the shared dashboards "
              "until this is unlocked.",
         note="Clear ACCOUNT: unlock plus team-wide impact."),
    dict(id="B05", scenario="parcel_lost", money=75.0, scope="single", workaround=False,
         plan="Pro", who="Owen Brady, subscriber",
         subject="Parcel marked delivered but never arrived",
         body="Tracking for order ORD-55210 says delivered on Tuesday, but nothing was left at my address. "
              "The carrier told me to contact the sender. The contents are worth USD 75.",
         note="Clear SHIPPING: lost parcel investigation."),
    dict(id="B06", scenario="app_crash", money=0.0, scope="single", workaround=True,
         plan="Team", who="Lucia Ferreira, analyst",
         subject="CSV export crashes the app on large reports",
         body="Exporting reports above roughly 10,000 rows crashes the desktop app every time. Smaller "
    ...
    dict(id="B07", scenario="address_update", money=0.0, scope="single", workaround=True,
         plan="Pro", who="Noah Kim, subscriber",
         subject="Change delivery address before dispatch",
         body="Please change the delivery address on order ORD-55988 to my office address before it ships. "
              "The order is still marked processing and nothing has shipped yet.",
         note="Clear SHIPPING: routine pre-dispatch update."),
    dict(id="B08", scenario="refund_request", money=480.0, scope="single", workaround=False,
         plan="Enterprise", who="Grace Lindqvist, finance manager",
         subject="Correct invoice INV-9021: USD 480 overage charge",
         body="INV-9021 charges USD 480 for API overages in August. Our own logs show 20,000 calls where "
    ...
    dict(id="B09", scenario="pricing_question", money=0.0, scope="single", workaround=True,
         plan="Free", who="Felix Braun, founder",
         subject="Pricing question: difference between Pro and Team plans",
         body="We are four people and want to know whether the Team plan includes the same API limits as "
              "Pro. No urgency, we are planning next quarter's budget.",
         note="Clear BILLING: pricing question, no action needed."),
    dict(id="B10", scenario="sso_loop", money=0.0, scope="team", workaround=False,
         plan="Enterprise", who="Amara Nwosu, IT manager",
         subject="SSO login loop for our whole team since this morning",
         body="Since about 08:00 UTC every member of our team of 30 authenticates with SSO successfully "
    ...
    dict(id="B11", scenario="mixed_charge_slow_app", money=89.0, scope="single", workaround=True,
         plan="Pro", who="Theo Marsh, consultant",
         subject="Duplicate charge and very slow dashboard",
         body="Two things: I was charged USD 89 twice for the same month, please refund one charge; and "
    ...
    dict(id="B12", scenario="performance", money=0.0, scope="team", workaround=True,
         plan="Team", who="Sofia Marchetti, engineering manager",
         subject="Dashboards slow for the whole team after yesterday's release",
         body="Since yesterday's release our dashboards take 12-15 seconds to load for all 8 team members. "
...
def build() -> dict:
    ...
    return {
        ...
        "counts": {"total": len(family_a) + len(family_b), "family_a": len(family_a),
                   "family_b": len(family_b), "ambiguous": len(ambiguous),
...
def main() -> int:
    ...
    print(f"wrote {args.out}: "
          f"{data['counts']['family_a']} payment-risk + {data['counts']['family_b']} support-triage cases, "
