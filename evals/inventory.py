...
def main() -> None:
    ...
    for wf in WORKFLOWS:
        ...
        for i, ex in enumerate(ev["examples"]):
            ...
            models = {m: sum(len(n.get("answers", {})) for n in v.get("nodes", []))
            ...
            grand_pairs += n_ref
        inv[wf] = {"n_cases_total": ev["n_cases"], "shipped_examples": len(ev["examples"]),
                   "catalog_questions": len(ev["questions"]), "cases": per_case}

    json.dump(inv, open(os.path.join(T, "inventory.json"), "w"), indent=1)

    print(f"{'workflow':<30} {'case':<40} {'in_chars':>9} {'ref_pairs':>9}  models(opus/sol/jev)  ref_nodes")
    print("-" * 140)
    total = 0
    for wf, data in inv.items():
        for c in data["cases"]:
            m = c["model_answers"]
            print(f"{wf:<30} {c['case_id'][:38]:<40} {str(c['input_chars']):>9} {c['reference_pairs']:>9}  "
                  f"{m.get('opus')}/{m.get('sol')}/{m.get('typesafe')}   {c['reference_nodes']}")
            total += c["reference_pairs"]
        print(f"  (catalog questions: {data['catalog_questions']}, total cases in full eval: {data['n_cases_total']})")
